...
@pytest.fixture
def sample_gchat_provider_webhook_afm() -> Path:
    return (
        Path(__file__).parent
        / "fixtures"
        / "sample_gchat_provider_webhook_agent.afm.md"
    )


@pytest.fixture
def sample_gchat_provider_webhook_sync_afm() -> Path:
    return (
        Path(__file__).parent
        / "fixtures"
        / "sample_gchat_provider_webhook_sync_agent.afm.md"
    )
...
class TestWebhookIntegration:
    ...
    @pytest.mark.asyncio
    async def test_gchat_provider_webhook_fire_and_forget(
        self,
        sample_gchat_provider_webhook_afm: Path,
    ) -> None:
        received_prompts: list[str] = []
        prompt_received = asyncio.Event()
        ...
        afm = parse_afm_file(sample_gchat_provider_webhook_afm, resolve_env=False)
        runner = LangChainRunner(afm, model=fake_llm)
        app = create_webhook_app(runner, auto_subscribe=False, verify_signatures=False)
    ...
    @pytest.mark.asyncio
    async def test_gchat_provider_webhook_sync_response(
        self,
        sample_gchat_provider_webhook_sync_afm: Path,
    ) -> None:
        fake_llm = FakeListChatModel(
            responses=['{"text": "Here is your deployment status."}']
        )
        afm = parse_afm_file(
            sample_gchat_provider_webhook_sync_afm, resolve_env=False
        )
        runner = LangChainRunner(afm, model=fake_llm)
        app = create_webhook_app(runner, auto_subscribe=False, verify_signatures=False)
